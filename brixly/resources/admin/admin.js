...
// admin notice
(function ($) {
    var $notice_container = $('.brixly-admin-big-notice--container');
    ...
    function disableNotice() {
        wp.ajax.post('brixly_disable_big_notice');
    }

    function toggleProcessing(value) {
        $(window).on('beforeunload.brixly-admin-big-notice', function () {
            return true;
        });
        if (value) {
            $('.brixly-admin-big-notice').addClass('processing');
            $('.brixly-admin-big-notice .action-buttons').fadeOut();
        } else {
            $('.brixly-admin-big-notice').removeClass('processing');
        }
    }
    ...
    function installBuilder(callback) {
        pluginNotice(brixly_builder_status.messages.installing);
        $.get(brixly_builder_status.install_url).done(function () {
            toggleProcessing(true);
            activateBuilder(callback);
        }).always(function () {
            $(window).off('beforeunload.brixly-admin-big-notice');
        });
    }

    function activateBuilder(callback) {
        pluginNotice(brixly_builder_status.messages.activating);

        wp.ajax.post('brixly_activate_plugin', { slug: 'brixly-page-builder' }).done(function () {
            $(window).off('beforeunload.brixly-admin-big-notice');
            if (callback) {
                callback();
            } else {
    ...
    function processBuilderInstalationStepts(callback) {
        wp.ajax.post('brixly_front_set_predesign', { index: selectedFrontPage });
        if (brixly_builder_status.status === "not-installed") {
            toggleProcessing(true);
            installBuilder(callback);
        }

        if (brixly_builder_status.status === "installed") {
            toggleProcessing(true);
            activateBuilder(callback);
        }
    }
    ...
    $notice_container.on('click', '.view-all-demos', function () {
        ...
        processBuilderInstalationStepts(function () {
            window.location = brixly_builder_status.view_demos_url;
        });
    });
