...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__undescore_extensions__ = __webpack_require__(37);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__preview_css_preview__ = __webpack_require__(346);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__preview_js_preview__ = __webpack_require__(347);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3__custom_preview__ = __webpack_require__(348);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_4__preview_selective_refresh__ = __webpack_require__(349);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_5__preview_specific_bindings__ = __webpack_require__(350);
...
var BrixlyCustomizerPreviewer = function () {
    function BrixlyCustomizerPreviewer() {
        _classCallCheck(this, BrixlyCustomizerPreviewer);
    }

    _createClass(BrixlyCustomizerPreviewer, null, [{
        key: "bindSetting",
    ...
    return BrixlyCustomizerPreviewer;
}();

window.BrixlyCustomizerPreviewer = BrixlyCustomizerPreviewer;
...
"use strict";
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__compare__ = __webpack_require__(59);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__maybe_deserialize__ = __webpack_require__(34);



var $ = jQuery,
    domready = __webpack_require__(23),
    sprintf = __webpack_require__(35).sprintf;

function isSettingActive(key) {
    if (!brixly_CONTROLS_ACTIVE_RULES[key]) {
        return true;
    }

    var rules = brixly_CONTROLS_ACTIVE_RULES[key] || [];
...
function getStyleRulesByMedia() {
    ...
    var _loop = function _loop(key) {
        if (!brixly_CSS_OUTPUT_CONTROLS.hasOwnProperty(key)) {
            return "continue";
        }
        ...
        var rules = brixly_CSS_OUTPUT_CONTROLS[key];
    ...
    for (var key in brixly_CSS_OUTPUT_CONTROLS) {
        var _ret = _loop(key);
...
function generateCSSOutputForMedia(media, data) {
    ...
    var selectorsPrefix = brixly_ADDITIONAL_JS_DATA.css_selectors_prefix || "";
...
function _displayStyle() {
    var $style = $('[data-brixly-theme-style="true"]'),
    ...
    for (var media in styleByMedia) {
        ...
        if (media === "__brixly__no__media__") {
            media = "";
        }
...
domready(function () {
    ...
    for (var key in brixly_CONTROLS_ACTIVE_RULES) {
        if (!brixly_CONTROLS_ACTIVE_RULES.hasOwnProperty(key)) {
            continue;
        }

        if (!brixly_CSS_OUTPUT_CONTROLS[key]) {
            continue;
        }

        for (var i = 0; i < brixly_CONTROLS_ACTIVE_RULES[key].length; i++) {
            var setting = brixly_CONTROLS_ACTIVE_RULES[key][i].setting;
    ...
    settingsInActiveRules.concat(_.keys(brixly_CSS_OUTPUT_CONTROLS)).forEach(function (control_id) {
...
"use strict";
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__js_helpers__ = __webpack_require__(58);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1_domready__ = __webpack_require__(23);
...
__WEBPACK_IMPORTED_MODULE_1_domready___default()(function () {
    ...
    settingsInActiveRules.concat(_.keys(brixly_JS_OUTPUT_CONTROLS)).forEach(function (control_id) {
        wp.customize(control_id, function (value) {
            value.bind(function (newValue, oldValue) {
                Object(__WEBPACK_IMPORTED_MODULE_0__js_helpers__["a" /* default */])(brixly_JS_OUTPUT_CONTROLS[this.id], newValue, oldValue);
                top.wp.customize.requestChangesetUpdate({}, { autosave: true });
            });
        });
    });
...
function isSelectiveRefreshSetting(id) {
    return brixly_ADDITIONAL_JS_DATA.selective_refresh_settings.indexOf(id) !== -1;
}
...
selectiveRefresh.Partial = selectiveRefresh.Partial.extend({
    ...
    ready: function ready() {
        ...
        _.each(this.placements(), function (placement) {
            _this2.addBrixlyOverlay(placement);
        });

        partialReady.apply(this, arguments);
    },
    addBrixlyOverlay: function addBrixlyOverlay(placement) {
        return;
        if (placement.container.children('span.customize-brixly-overlay').length === 0) {
            placement.container.append('<span class="customize-brixly-overlay"></span>');
        }
    },
    renderContent: function renderContent(placement) {
        var _this3 = this;

        this.addBrixlyOverlay(placement);
...
(function ($) {
    ...
    function setHeroEditButtonTop() {
        var top = '0';
        var $hero = $("#hero");
        var overlap = $("[data-brixly-navigation-overlap]").hasClass('h-navigation_overlap');
        if (overlap) {
    ...
    window.brixlyUpdateHeroPenPosition = setHeroEditButtonTop;
...
function focus(item, newValue, oldValue) {
    ...
    entity.collapse();
    top.wp.customize.trigger('brixly_sections_collapse');
    entity.focus();
    //api.preview.send( 'focus-control-for-setting', entity_id );
}

function brixlyComponentToggle(item, newValue, oldValue) {

    var el = $(item.selector);
    var component = el.data()['fn.brixly.' + item.value];
    if (component) {
        if (newValue) {
            component.start();
        } else {
        ...
        console.log(item.selector, 'has no brixly component');
    }
}

function brixlyComponentRestart(item, newValue, oldValue) {
...
function brixlyNavigationToggleSticky(item, newValue, oldValue) {
    ...
    if (newValue == false) {
        ...
        component.opts.data.sticky = window.brixlyNavStickyOpts;
    }
...
function brixlyNavigationToggleOverlap(item, newValue, oldValue) {
...
function brixlySetAttr(item, newValue, oldValue) {
    var el = $(item.selector);
    ...
    data.forEach(function (item) {
        switch (item.action) {
            ...
            case 'focus':
                ...
                break;
            case 'brixly-component-restart':
                brixlyComponentRestart(item, newValue, oldValue);
                break;
            case 'brixly-component-toggle':
                brixlyComponentToggle(item, newValue, oldValue);
                break;
            case 'brixly-navigation-toggle-sticky':
                brixlyNavigationToggleSticky(item, newValue, oldValue);
                break;
            case 'brixly-navigation-toggle-overlap':
                brixlyNavigationToggleOverlap(item, newValue, oldValue);
                break;
            case 'brixly-set-attr':
                brixlySetAttr(item, newValue, oldValue);
                break;
        }
    });
}
