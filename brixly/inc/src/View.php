...
class View {
	...
	public static function make( $path, $data = array() ) {
		...
		$wp_query->query_vars['brixly_data'] = new Tree( $data );
		...
		$wp_query->query_vars['brixly_data'] = null;
	}

	public static function getData( $path, $default = null ) {
		global $wp_query;
		$brixly_data = $wp_query->query_vars['brixly_data'];
		if ( $brixly_data ) {
			/** @var \BrixlyWP\Theme\Core\Tree $brixly_data */
			return $brixly_data->findAt( $path, $default );
		}
	...
	public static function printMenu( $attrs, $walker = "" ) {
		...
		$drop_down_menu_classes = apply_filters( 'brixly_primary_drop_menu_classes', array( 'brixly-menu' ) );
		...
		wp_nav_menu( array(
			...
			'container_class' => 'brixly-menu-container',
	...
	public static function menuFallback( $attrs, $walker = '' ) {
		...
		return wp_page_menu( array(
			"menu_class" => 'brixly-menu-container',
			'before'     => '<ul class="' . esc_attr( implode( " ", $drop_down_menu_classes ) ) . '">',
			'after'      => apply_filters( 'brixly_nomenu_after', '' ) . "</ul>",
			'walker'     => $walker,
		) );
	}
	...
	public static function printContentWrapperAttrs( $classes = array() ) {
		...
		$classes = Hooks::brixly_apply_filters( 'content_wrapper_class', $classes );
        ...
        <a href="<?php the_permalink(); ?>">
            <div class="image-container-frame">
				...
				if ( has_post_thumbnail() ) {
					...
					?>
                    <svg class="brixly-post-list-item-thumb-placeholder <?php echo esc_attr( $classes ); ?>"
	...
	public static function printPagination( $args = array(), $class = 'pagination' ) {
		...
		$args = wp_parse_args( $args, array(
			'mid_size'           => 2,
			'before_page_number' => '<span class="meta-nav screen-reader-text">' . __( 'Page', 'brixly' )
			                        . ' </span>',
			'prev_text'          => __( '<i class="fa fa-angle-left" aria-hidden="true"></i>', 'brixly' ),
			'next_text'          => __( '<i class="fa fa-angle-right" aria-hidden="true"></i>', 'brixly' ),
			'screen_reader_text' => __( 'Posts navigation', 'brixly' ),
		) );
